...
contract('InitializedState.js: check transitions', function(accounts) {
  ...
  describe("depositFunds", async function() {
     ...
     */
    it("1.5 if deposit >,=,< maxPayout and timeout happened without policy violated, switch to Cancelled",
      async function() {
     ...
     */
    it("1.1 if deposit >,=,< maxPayout stay in this state if timeout did not happen",
      async function() {
        await qspb.depositFunds(firstPoolId, 0, {from : stakeholder});
        await assertPoolState(firstPoolId, PoolState.Initialized);
  ...
  describe("withdrawDeposit", async function() {
     ...
     */
    it("1.6 switch to Cancelled if the policy is not violated and timeout did not happen",
      async function() {
     ...
     * pool was cancelled.
     */
    it("1.6 switch to Cancelled even if the policy is violated",
      async function() {
        await policy.updateStatus(true);
        await qspb.withdrawDeposit(firstPoolId, {from : stakeholder});
        await assertPoolState(firstPoolId, PoolState.Cancelled);
     ...
     */
    it("1.6 if timeout happend with policy not being violated should also switch into Cancelled",
      async function() {
  ...
  describe("withdrawStake", async function() {
     ...
     */
    it("1.1 if timeout did not happen and policy is not violated, remain in this state",
      async function() {
     ...
     * the the pool remained in the current state.
     */
    it("1.1 zero stake, if timeout did not happen and policy is not violated, remain in state 1",
      async function() {
        await qspb.withdrawStake(firstPoolId, {from : staker});
        await assertPoolState(firstPoolId, PoolState.Initialized);
      }
  ...
  describe("checkPolicy", async function() {
     ...
     */
    it("1.7 if policy is violated switch state to cancelled",
      async function() {
        await policy.updateStatus(true);
  ...
  describe("stakeFunds", async function() {
     ...
     */
    it("1.5 cancel if timeout happened",
      async function() {
     ...
     * no change.
     */
    it("1.2 stays in the same state if stake is too low, policy is not violated, and timeout did not happen",
      async function() {
        await token.approve(qspb.address, 13, {from : staker});
        await qspb.stakeFunds(firstPoolId, 13, {from : staker});
        await assertPoolState(firstPoolId, PoolState.Initialized);
        // deposit should not matter
        let leftToDeposit = pool.maxPayoutQspWei.sub(pool.depositQspWei);
        await token.approve(qspb.address, leftToDeposit, {from : stakeholder});
        await qspb.depositFunds(firstPoolId, leftToDeposit, {from : stakeholder});
        // stake again
        await qspb.stakeFunds(firstPoolId, 0, {from : staker});
        await assertPoolState(firstPoolId, PoolState.Initialized);
      }
    );
     ...
     */
    it("1.4 if the sum of stakes is >= minStake and deposit is large enough, transition to NotViolatedFunded.",
      async function() {
        // fund pool
        let leftToDeposit = pool.maxPayoutQspWei.sub(pool.depositQspWei);
        await token.approve(qspb.address, leftToDeposit, {from : stakeholder});
     ...
     */
    it("1.3 if the sum of stakes is >= minStake and deposit is not large enough, transition to NotViolatedUnderfunded.",
      async function() {
        await token.approve(qspb.address, pool.minStakeQspWei, {from : staker});
        // stake just not enough
        await qspb.stakeFunds(firstPoolId, pool.minStakeQspWei.sub(1), {from : staker});
